...
class BuyerModel extends CI_Model
{
      ...
      public function get_all_advertisements($id, $data)
      {
            ...
            $condition = "SELECT `advertisements`.`advertisement_id`,`advertisements`.`advertisement_name`,`advertisements`.`created_by`,`advertisements`.`actual_price`,`advertisements`.`offer_price`,`advertisements`.`cashback_per_user`, `offerforx` , `advertisements`.`status`,`advertisements`.`min_user_count`,`advertisements`.`quantity_per_user`, date_format(`advertisements`.`end_date`, '%e %M %Y') as end_date, `advertisement_statics`.`views_count`, `advertisement_statics`.`group_count`, date_format(`advertisements`.`start_date`, '%e %M %Y') as start_date, `advertisements`.`advertisement_details`, `advertisement_statics`.`views_count`, `advertisement_statics`.`group_count`, `categories`.`category_title`, `subcategories`.`subcategory_title`, `subcategories2`.`subcategory_title2`, `subcategories3`.`subcategory_title3`, `subcategories4`.`subcategory_title4`, `subcategories5`.`subcategory_title5`, `advertisements`.`location`
FROM `advertisements`
...
AND (`advertisements`.`end_date` >= '" . $currentdate . "'))";
      ...
      public function get_all_favadvertisements($id)
      {
            ...
            $condition = "SELECT `advertisements`.`advertisement_id`,`advertisements`, `offerforx` .`advertisement_name`,`advertisements`.`actual_price`,`advertisements`.`offer_price`, `advertisements`.`cashback_per_user`,`advertisements`.`status`, date_format(`advertisements`.`end_date`, '%e %M %Y') as end_date, `advertisement_statics`.`views_count`, `advertisement_statics`.`group_count`, date_format(`advertisements`.`start_date`, '%e %M %Y') as start_date, `advertisements`.`advertisement_details`, `advertisement_statics`.`views_count`, `advertisement_statics`.`group_count`, `categories`.`category_title`, `subcategories`.`subcategory_title`, `subcategories2`.`subcategory_title2`, `subcategories3`.`subcategory_title3`, `subcategories4`.`subcategory_title4`, `subcategories5`.`subcategory_title5`, `advertisements`.`location`
FROM `advertisements`
LEFT JOIN `advertisement_statics` ON `advertisement_statics`.`advertisement_id` = `advertisements`.`advertisement_id`
LEFT JOIN `categories` ON `categories`.`category_id` = `advertisements`.`category_id`
LEFT JOIN `subcategories` ON `subcategories`.`subcategory_id` = `advertisements`.`subcategory_id`
...
AND (`advertisements`.`end_date` >= '" . $currentdate . "'))";
      ...
      {
            return $this->db->select('advertisements.advertisement_id,advertisements.quantity_per_user,advertisements.offerforx,advertisements.advertisement_name,advertisements.cashback_per_user,advertisements.actual_price,advertisements.offer_price,advertisements.status,advertisements.min_user_count,date_format(advertisements.start_date,"%e %M %Y") as start_date,date_format(advertisements.end_date,"%e %M %Y") as end_date,advertisements.advertisement_details,advertisement_statics.views_count,advertisement_statics.group_count,categories.category_title,subcategories.subcategory_title,subcategories2.subcategory_title2,subcategories3.subcategory_title3,subcategories4.subcategory_title4,subcategories5.subcategory_title5,advertisements.location')
